...
#==================================== Main Script ====================================
TITLE="NVIDIA Turing (GTX 16xx, RTX 20xx) and newer - Driver Installer"
...
if whiptail --title "$TITLE" --yesno "Remove unsed kernels?\n\nThis is Highly reccomended\n\nThis will speed-up the install procces." $HEIGHT $WIDTH; then
    ...
    echlog "Skipping kernel removal in nvidia installer."
fi

PACKAGES=(nvidia nvidia-opencl nvidia-libs-32bit nvidia-container-toolkit nvidia-vaapi-driver vulkan-loader vulkan-loader-32bit Vulkan-Headers Vulkan-Tools libspa-vulkan libspa-vulkan-32bit)
## Menu list entries:
### "PACKAGE" "DESCRIPTION" "OFF/ON"
### OFF/ON refers if the menu item will be automaticly selected(ON) or de-selected(OFF)
MANUAL_OPTIONS=(
    #"PACKAGE"                  "DESCRIPTION"               "OFF/ON"
    "nvidia"                    "NVIDIA drivers for linux - Libraries and Utilities" ON
    "nvidia-opencl"             "NVIDIA drivers for linux - OpenCL implementation" ON
    "nvidia-libs-32bit"         "NVIDIA drivers for linux - common libraries (32bit)" ON
