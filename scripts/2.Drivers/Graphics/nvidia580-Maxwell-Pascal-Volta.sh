...
#==================================== Main Script ====================================
TITLE="NVIDIA Maxwell, Pascal and Volta cards (Geforce 900/1000) - Driver Installer"
...
if whiptail --title "$TITLE" --yesno "Remove unsed kernels?\n\nThis is Highly reccomended\n\nThis will speed-up the install procces." $HEIGHT $WIDTH; then
    ...
    echlog "Skipping kernel removal in nvidia580 installer."
fi

PACKAGES=(nvidia580 nvidia580-opencl nvidia580-libs-32bit nvidia-container-toolkit nvidia-vaapi-driver vulkan-loader vulkan-loader-32bit Vulkan-Headers Vulkan-Tools libspa-vulkan libspa-vulkan-32bit)
## Menu list entries:
### "PACKAGE" "DESCRIPTION" "OFF/ON"
### OFF/ON refers if the menu item will be automaticly selected(ON) or de-selected(OFF)
MANUAL_OPTIONS=(
    #"PACKAGE"                  "DESCRIPTION"               "OFF/ON"
    "nvidia580"                 "NVIDIA drivers for linux - Libraries and Utilities" ON
    "nvidia580-opencl"          "NVIDIA drivers for linux - OpenCL implementation" ON
    "nvidia580-libs-32bit"      "NVIDIA drivers for linux - common libraries (32bit)" ON
